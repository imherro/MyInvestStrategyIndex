...
`/cashflow-growth-compare` 参照 `/value-compare` 的完整交互，只保留 480092.CNI 自由现金流R收益指数和 CN2296.CNI 创成长R收益指数两个真实指数，同时保留基于当前勾选成分动态计算的双指数等权组合、滚动60日风险平价组合、最新风险平价比例、上证指数灰色背景线、曲线/回撤/散点/指标排序和更新数据功能。
